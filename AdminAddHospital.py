...
class AddHospital:
    ...
    def __init__(self):
        ...
        Label(self.root, text="Add Hospital", font=("Ubuntu Bold", 16), fg="#707070", bg="white").place(x=250, y=40)
        ...
        self.textInput("Email", 200, 280, y + 180, 'entry_Email')
    ...
    def show(self):
        ...
        try:
            ...
            cursor.execute(f"""
                        insert into Hospital values ({id}, "{Name}", "{Current_Address}",
                        "{Current_District}", "{Current_Division}", "{Phone}", "{Email}"
                        )
                    """)
